/**
 * ModelMarketplacePremium - Ultimate Device-Adaptive AI Model Store
 * 
 * Premium version with:
 * - Full theme system integration
 * - Enhanced haptic feedback sequences
 * - Platform-specific animations
 * - Improved "Choose Any" with visual warnings
 * - Real-time device status monitoring
 * - Progress ring with verification states
 * 
...
import {
  ...
  Play, Cpu, Battery, Gauge, Shield, Filter, SlidersHorizontal,
  AlertTriangle, Info, Lock, Unlock, CheckCircle2
} from 'lucide-react';
...
import { useTheme, useAnimationConfig } from '../contexts/ThemeContext';
import { createLogger } from '../utils/logger';

const log = createLogger('ModelMarketplacePremium');
...
const calculateCompatibilityScore = (model, deviceProfile) => {
  ...
  // Performance match (0-20 points)
  const qualityMatch = (model.qualityRating || 3) / 5;
  const speedMatch = (model.speedRating || 3) / 5;
  ...
  // Battery consideration (0-15 points)
...
const getWarningLevel = (model, deviceProfile) => {
  ...
  if (model.size > recommendations.maxModelSize * 2) {
    return {
      level: 'critical',
      severity: 'critical',
      message: `This model requires ${model.sizeDisplay} but your device supports up to ${Math.round(recommendations.maxModelSize * 2 / 1024 / 1024 / 1024 * 10) / 10}GB.`,
      canRun: false,
    };
  }
  ...
  if (model.size > recommendations.maxModelSize * 1.5) {
    return {
      level: 'severe',
      severity: 'warning',
      message: 'This model is much larger than recommended. Expect very slow performance and high battery usage.',
      canRun: true,
      batteryImpact: 'high',
  ...
  if (runtime.battery.isLowPower && model.size > 600 * 1024 * 1024) {
    return {
      level: 'battery',
      severity: 'info',
      message: 'Battery is low. This model will drain battery faster.',
      canRun: true,
      batteryImpact: 'medium',
...
const ProgressRing = ({ progress, size = 48, status }) => {
...
const CategoryBadge = ({ category }) => {
  const configs = {
    lightweight: { icon: Zap, label: 'Fast', color: 'green' },
    balanced: { icon: Gauge, label: 'Balanced', color: 'blue' },
    quality: { icon: Brain, label: 'Quality', color: 'purple' },
  };
  ...
  return (
    <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-${config.color}-500/10 border border-${config.color}-500/20 backdrop-blur-sm`}>
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="glass-card-premium max-w-md w-full p-6 space-y-4"
        ...
        <div className="bg-white/5 rounded-xl p-4 space-y-2">
          ...
          {warning.batteryImpact && (
            <div className="flex justify-between text-sm">
              ...
              <span className={`font-medium ${
                warning.batteryImpact === 'high' ? 'text-red-400' :
          ...
          {warning.performanceImpact && (
            <div className="flex justify-between text-sm">
              ...
              <span className={`font-medium ${
                warning.performanceImpact === 'severe' ? 'text-red-400' :
...
const ModelCard = ({
  ...
  useEffect(() => {
    const analyze = () => {
      ...
      const warning = getWarningLevel(model, deviceProfile);
      
      let estimatedSpeed = null;
      if (warning.canRun) {
        const tier = deviceProfile?.recommendations?.tier || 'standard';
        const baseSpeed = { essential: 15, standard: 25, advanced: 45, pro: 80 }[tier] || 25;
        const sizeFactor = Math.max(0.3, 1 - (model.size / 2000000000));
        estimatedSpeed = Math.round(baseSpeed * sizeFactor);
      }
      
      setCompatibilityInfo({ score, warning, estimatedSpeed, canRun: warning.canRun });
    };
    analyze();
  }, [model, deviceProfile]);
  ...
  const handleDelete = async () => {
    await HapticsService.sequence.delete();
    onDelete(model.id);
    ...
    <>
      ...
      >
        ...
        <div className="relative">
          <div className="flex items-start justify-between mb-4">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap mb-2">
                <h3 className="font-bold text-lg text-white truncate">{model.name}</h3>
          ...
          {(model.qualityRating || model.speedRating) && (
          ...
          <div className="flex gap-3">
            {isDownloading ? (
              <div className="flex items-center gap-4 flex-1 py-2">
                ...
                />
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-semibold text-white">{downloadStatus || 'Downloading...'}</div>
                  <div className="text-xs text-slate-500 mt-0.5 font-medium">{Math.round(downloadProgress)}% complete</div>
                </div>
              </div>
              ...
              <>
                <motion.button
                  whileHover={{ scale: isActive ? 1 : animationConfig.scale.hover }}
                  whileTap={{ scale: isActive ? 1 : animationConfig.scale.tap }}
                ...
                >
                  {isActive ? (
                    ...
                    <><ChevronRight size={18} /> Use Model</>
                  )}
                </motion.button>
              ...
              <motion.button
                whileHover={{ scale: isIncompatible ? 1 : animationConfig.scale.hover }}
                whileTap={{ scale: isIncompatible ? 1 : animationConfig.scale.tap }}
    ...
    <div className="glass-card p-4 mb-6">
      <div className="flex flex-wrap items-center gap-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-slate-200">{canRunAI ? 'AI Ready' : 'AI Unavailable'}</span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-slate-200 capitalize">{recommendations.tier} Tier</span>
        </div>
...
const ModelMarketplacePremium = () => {
  const navigate = useNavigate();
  const { platform, isNative } = useTheme();
    ...
    const init = async () => {
      try {
        ...
        const [profile] = await Promise.all([
          DeviceCapabilityProfiler.getProfile(),
          AIModelManager.init(),
        ]);
  ...
  const checkResumeInfo = async (models) => {
    ...
    for (const model of models) {
      try {
        ...
        if (resumeInfo?.canResume) resumeMap.set(model.id, resumeInfo);
      } catch (error) {
  ...
  const processedModels = useMemo(() => {
    ...
    if (filter === 'installed') {
      ...
      filtered = models.filter((m) => calculateCompatibilityScore(m, deviceProfile) >= 30);
    }
    
    return filtered.sort((a, b) => {
      switch (sortBy) {
        case 'compatibility':
          return calculateCompatibilityScore(b, deviceProfile) - calculateCompatibilityScore(a, deviceProfile);
        case 'speed':
          return (b.speedRating || 0) - (a.speedRating || 0);
        case 'quality':
  ...
  const handleDownload = async (modelId) => {
    ...
    if (result.success) {
      ...
      await HapticsService.sequence.downloadComplete();
    } else if (result.canResume) {
      const resumeInfo = await AIModelManager.getResumeInfo(modelId);
      if (resumeInfo) setResumeInfoMap((prev) => new Map(prev).set(modelId, resumeInfo));
    }

    setDownloadingModel(null);
  };

  const handleResume = async (modelId) => await handleDownload(modelId);
  ...
  const handleToggleShowAll = async () => {
    ...
    await HapticsService.toggle(!showAllModels);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto pb-24 px-4">
      ...
      >
        ...
        <p className="text-slate-400">Download AI models optimized for your device. Models run entirely offline.</p>
      </motion.div>
        ...
        {[
          ...
          >
            ...
            {tab.id === 'installed' && installedModels.size > 0 && (
              <span className="ml-1 text-xs px-1.5 py-0.5 rounded-full bg-white/20">{installedModels.size}</span>
            )}
          </motion.button>
        ))}
      </div>
        ...
        <AnimatePresence mode="popLayout">
          {processedModels.length === 0 ? (
            ...
            >
              ...
              <p className="text-sm text-slate-500">
                {filter === 'installed' ? 'Download models to use AI features offline' : 'Try adjusting your filters'}
              </p>
            </motion.div>
          ) : (
...
export default ModelMarketplacePremium;