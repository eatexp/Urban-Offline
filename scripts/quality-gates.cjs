...
class QualityChecker {
  ...
  async run() {
    ...
    // Check 1: Headless Toolchain Integration (Semgrep + Playwright)
    await this.checkHeadlessToolchain();

    // Check 2: Design System Compliance
    await this.checkDesignSystem();

    // Check 3: Performance Budgets
    await this.checkPerformance();

    // Check 4: Architectural Compliance
    await this.checkArchitecture();

    // Check 5: Offline-First Verification
    await this.checkOfflineFirst();

    // Check 6: Pillar Enforcement
  ...
  /**
   * Check headless toolchain integration (Semgrep + Playwright)
   * Compliance: .clinerules §9 - Headless Toolchain & Automated Quality Gates
   */
  async checkHeadlessToolchain() {
    console.log('🔧 Checking Headless Toolchain Integration...');

    try {
      const { execSync } = require('child_process');
      
      // Check 1: Semgrep installation
      try {
        const semgrepVersion = execSync('semgrep --version', { encoding: 'utf8' }).trim();
        console.log(`  ✓ Semgrep: ${semgrepVersion}`);
      } catch (error) {
        this.addIssue(
          'system',
          'Semgrep not installed or not in PATH',
          'error',
          'toolchain'
        );
        return; // Skip further checks if Semgrep is missing
      }
      ...
      try {
        ...
        this.addIssue(
          'system',
          'Playwright not installed or not in PATH',
          'warning',
          'toolchain'
        );
      }
      ...
      const requiredConfigs = [
        { path: '.semgrep.yaml', name: 'Semgrep configuration' },
        { path: '.semgrepignore', name: 'Semgrep ignore file' },
        { path: 'playwright.config.ts', name: 'Playwright configuration' },
        { path: 'tests/smoke.spec.ts', name: 'Playwright smoke test' }
      ];
      ...
      for (const config of requiredConfigs) {
        if (fs.existsSync(config.path)) {
          ...
          this.addIssue(
            ...
            config.path.includes('smoke.spec.ts') ? 'warning' : 'error',
            'toolchain'
          );
        }
      }

      // Check 4: JSON reporter configuration in Playwright
      const playwrightConfig = fs.readFileSync('playwright.config.ts', 'utf8');
      if (!playwrightConfig.includes('json')) {
        this.addIssue(
          'playwright.config.ts',
          'Playwright config missing JSON reporter (required for automated parsing)',
          'warning',
          'toolchain'
        );
      } else {
        console.log('  ✓ Playwright JSON reporter configured');
      }

      // Check 5: .semgrepignore has required exclusions
      ...
      if (missingExclusions.length > 0) {
        this.addIssue(
          '.semgrepignore',
          `Missing required exclusions: ${missingExclusions.join(', ')}`,
          'warning',
          'toolchain'
        );
      } else {
        console.log('  ✓ Semgrep ignore file properly configured');
      }

      console.log('  ✓ Headless toolchain integration verified');
    } catch (error) {
      this.addIssue(
        'system',
        `Headless toolchain check failed: ${error.message}`,
        'error',
        'toolchain'
      );
    }
